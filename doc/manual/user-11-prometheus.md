...
# Prometheus Metrics for OS Kernel Version

pgagroal exposes kernel version information through three distinct Prometheus metrics, depending on the underlying operating system:

| Operating System       | Metric Name          | Included Labels                      | Availability Conditions                                       |
|------------------------|----------------------|--------------------------------------|--------------------------------------------------------------|
| **Linux**             | `pgagroal_os_linux`   | `os`, `major`, `minor`, `patch`      | Available if kernel version retrieval succeeds               |
| **macOS**             | `pgagroal_os_macos`   | `os`, `major`, `minor`, `patch`      | Available if kernel version retrieval succeeds               |
| **BSD (FreeBSD, OpenBSD)** | `pgagroal_os_bsd` | `os`, `major`, `minor`               | Available if kernel version retrieval succeeds (patch not available) |
...
The `pgagroal_os_bsd` metric reports the kernel version for **BSD-based** systems (such as FreeBSD and OpenBSD).  
...
### Example Output:
```text
# HELP pgagroal_os_bsd Kernel version as major.minor
...
- If kernel version retrieval **fails** (e.g., unsupported system call, permission issues), **no metric will be emitted**.
