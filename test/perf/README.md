...
A per-PR performance check that detects throughput/latency regressions, run by
...
**`perf-results`** artifact (the per-run JSONs). No PR comment is posted — a
`pull_request` workflow from a fork has no write access, and running fork code
with write permissions is unsafe. If automated PR comments are wanted later, add
a separate trusted `workflow_run` workflow that only reads the `perf-results`
