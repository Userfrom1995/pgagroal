...
# Ensure baseline
sed "s/^log_level = .*/log_level = info/" "$CONF_FILE" > "${CONF_FILE}.tmp" && mv "${CONF_FILE}.tmp" "$CONF_FILE"
set_main_port "$ORIG_PORT"
$CLI -c "$CONF_FILE" conf reload > /dev/null 2>&1
sleep 1

echo "[CASE 1] HOT reload from file (log_level)"
sed "s/^log_level = .*/log_level = debug/" "$CONF_FILE" > "${CONF_FILE}.tmp" && mv "${CONF_FILE}.tmp" "$CONF_FILE"
$CLI -c "$CONF_FILE" conf reload > /dev/null 2>&1
sleep 1
...
sed "s/^log_level = .*/log_level = warn/" "$CONF_FILE" > "${CONF_FILE}.tmp" && mv "${CONF_FILE}.tmp" "$CONF_FILE"
set_main_port "9999"
$CLI -c "$CONF_FILE" conf reload > /dev/null 2>&1
sleep 1
